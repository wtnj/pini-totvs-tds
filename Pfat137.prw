...
MHORA      := TIME()
WNREL    := "PFAT137_" + SUBS(CUSUARIO,7,3)+SUBS(MHORA,1,2)+SUBS(MHORA,7,2)
CSTRING  := "PGAUT"
...
WHILE !EOF() .AND. SE3->E3_FILIAL == xFilial("SE3") .and. DTOS(SE3->E3_DATA) <= DTOS(MV_PAR02) .and. !lEnd   
	...
	WHILE  !Eof() .and. SD2->D2_FILIAL == xFilial("SD2") .AND. MNOTA == SD2->D2_DOC .AND. CONTINUA .and. SD2->D2_SERIE = MSERIE //20040713
		...
		If !(SD2->D2_SERIE)$ "UNI/CUP/CFS/CFA/CFB/CFC/CFD/ANG/CFE/NFS/SEN/STD/8  " .or. SD2->D2_PEDIDO <> MPEDIDO //20041227 CFA 20050520 CFB //20060403 CFD //20061031 ANG //20070315 CFE //20070328 NFS //20080220 SEN //20081031 STD
			DbSelectArea("SD2")
			DbSkip()
			Loop
		EndIf
...
COPY TO PGAUT.DBF VIA "DBFCDXADS" // 20121106 
