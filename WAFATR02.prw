...
Do While !EOF()
    ...
    AADD(aDadosBanco,SUBSTR(cNumCon,Len(cNumCon),1))	// [5]Dígito da conta corrente
    AADD(aDadosBanco," ")                               // [6]Codigo da Carteira
                                             
//	If Empty(SA1->A1_ENDCOB) //20080521 Danilo C S Pala
		aDatSacado   := {}
		AADD(aDatSacado,AllTrim(SA1->A1_NOME))                          	// [1]Razão Social
		AADD(aDatSacado,AllTrim(SA1->A1_COD )+"-"+SA1->A1_LOJA)           // [2]Código
		AADD(aDatSacado,AllTrim(ExecBlock("VCB01"))+" - "+AllTrim(ExecBlock("VCB02"))) // [3]Endereço
...
/*	Else
		...
		AADD(aDatSacado,AllTrim(SA1->A1_NOME))            	 				  	// [1]Razão Social
		AADD(aDatSacado,AllTrim(SA1->A1_COD )+"-"+SA1->A1_LOJA)               	// [2]Código
		AADD(aDatSacado,AllTrim(SA1->A1_ENDCOB)+"-"+AllTrim(SA1->A1_BAIRROC))  	// [3]Endereço
...
*/	//20080521 Danilo C S Pala
	...
	Else
		If MsgYesNo("Uma nova data de vencimento foi digitada, deseja utilizá-la?(Sim) Para deixar o vencimento original do título(Não)")
	...
	aDadosTit	:= {}
	AADD(aDadosTit,AllTrim(E1_NUM)+AllTrim(E1_PARCELA))       									   // [1] Número do título
	AADD(aDadosTit,E1_EMISSAO)                               									   // [2] Data da emissão do título
	AADD(aDadosTit,dDataBase)                    												      // [3] Data da emissão do boleto
	AADD(aDadosTit,dNovoVencto) //E1_VENCTO )20130723			                                 // [4] Data do vencimento
//	AADD(aDadosTit,(E1_SALDO - nVlrAbat))                  	 // [5] Valor do título
	AADD(aDadosTit,ROUND(E1_VALOR - nVlrAbat,2))             									   // [5] Valor do título
	AADD(aDadosTit,aCB_RN_NN[3])								                                    // [6] Nosso número (Ver fórmula para calculo)
...
//Solicitado pela Bianca em 07/02/07 - André Luis - Introde % Juros no Cad.Cliente p/Boleto
...
//CMULTA := If(aDadosTit[10]<=0,0,aDadosTit[10]) //20080520: Danilo C S Pala comentado
	...
	aBolText 	:= {}
//	AADD(aBolText,"Após o vencimento cobrar R$ " + AllTrim(Transform(((aDadosTit[5]*0.05)/30),"@E 99,999.99")) + " por dia de atraso.")
/*	AADD(aBolText,"Após o vencimento cobrar R$ " + AllTrim(Transform(((aDadosTit[5]*cJuros)/30),"@E 99,999.99")) + " por dia de atraso.")
	AADD(aBolText,"Após o vencimento cobrar "    + AllTrim(Transform(CMULTA                    ,"@E 99,999")) + " % ao mês de atraso.")
	...
	AADD(aBolText,"NÃO ACEITAMOS PAGAMENTOS VIA DOC, TRANSFERÊNCIA OU DEPÓSITO") //20130627  SIMPLES
	//AADD(aBolText,"POIS NOSSO SISTEMA NÃO IDENTIFICA ESTAS FORMAS DE PAGAMENTO") 20130627
	...
	If SM0->M0_CODIGO=='01' //ED  DAQUI 20130627
		AADD(aBolText,"DÚVIDAS, 2ª VIA DE BOLETO, ATUALIZAÇÃO OU PRORROGAÇÃO LIGUE PARA (11) 2173-2445")
	Elseif SM0->M0_CODIGO=='03' //BP
		AADD(aBolText,"DÚVIDAS, 2ª VIA DE BOLETO, ATUALIZAÇÃO OU PRORROGAÇÃO LIGUE PARA (11) 2173-2443")
...
oPrint:Say  (nRow2+0725,400 ,"ATÉ O VENCIMENTO PAGÁVEL EM QUALQUER BANCO",oFont10)  //Danilo C S Pala 20080520
oPrint:Say  (nRow2+0765,400 ,"APÓS O VENCIMENTO PAGÁVEL APENAS NO BANCO ITAU",oFont10) //Danilo C S Pala 20080520
...
oPrint:Say  (nRow3+2015,400 ,"ATÉ O VENCIMENTO PAGÁVEL EM QUALQUER BANCO",oFont10)  //Danilo C S Pala 20080521
oPrint:Say  (nRow3+2055,400 ,"APÓS O VENCIMENTO PAGÁVEL APENAS NO BANCO ITAU",oFont10) //Danilo C S Pala 20080521
...
DbSelectArea("SE1")
RecLock("SE1",.f.)
SE1->E1_NUMBCO 	:=	cNroDoc + cDigDoc //subs(aCB_RN_NN[3],4,8)+subs(aCB_RN_NN[3],13,1)   // Nosso número (Ver fórmula para calculo) //20101014 //20101108
