#include "rwmake.ch"
#INCLUDE "TOPCONN.CH"
/*/ alterado 20040329 por Danilo C S Pala
...
//Danilo C S Pala 20060911: revista = produto
//Danilo C S Pala 20071010: ajustes para mudanca de layout, solicitado por Cicero
...
±±³Adaptado  ³ PFAT071B ³ Autor ³ Gilberto A. de Oliveira  ³ Data ³ 05/05/00 ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
±±³Descricao ³ Nota Fiscal de Saida. BP S/A                                  ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
±±³Uso       ³ Especifico para Editora PiniWeb.                              ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
SetPrvt("_NLIN07,_NLIN08,_NLIN09,_NLIN10,_NLIN11,_NLIN12")
SetPrvt("_NAJUSTE,XTXTCF,BB,XY2,_CC5NUM,_NIENDCOB")
SetPrvt("XY1,NTAMDET,XB_ICMS_SOL,XV_ICMS_SOL,_LIMP,NTAMMEN")
SetPrvt("_NTTLOBS,NTAMOBS,_SALIAS,AREGS")    
SetPrvt("NVALCOFINS, NVALPIS, NVALCSLL, BRETEM, B_SERVENG, NVALRETTOTAL, NVALIRRF, NVALORTOTAL") //20040212
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
Processa({|| RptDetail()})// Substituido pelo assistente de conversao do AP5 IDE em 30/03/02 ==>     RptStatus({|| Execute(RptDetail)})
...
±±ºPrograma  ³RptDetail ºAutor  ³Microsiga           º Data ³  04/01/02   º±±
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
ProcRegua(Abs(Val(mv_par02)-Val(mv_par01))+1)
...
While !Eof() .And. (SF2->F2_DOC <= mv_par02) .And. (SF2->F2_FILIAL==_xFilial) .And. lContinua
	...
	While !Eof() .And. (SD2->D2_DOC == xNUM_NF) .And. (SD2->D2_SERIE == xSERIE) .And. (SD2->D2_FILIAL==_xFilD2)
	...
	For I:=1 to Len(xCOD_PRO)
		...
		IF SUBS(XCOD_PRO[I],1,7)=="3800001"
			AADD(xCOD_TRIB  , "00")
			XSUP_INF:= .T.
		ELSEIF SUBS(XCOD_PRO[I],1,7)=="3800002"
			AADD(xCOD_TRIB  , "00")
			XSUP_INF:= .T.
		ELSEIF SUBS(XCOD_PRO[I],1,7)=="3800003"
			AADD(xCOD_TRIB  , "00")
		ELSE
		...
		Do Case
		Case SB1->B1_TIPO=="CD"            // Para CD/DISK.
			AADD(xSERVICO,"N")
		Case SB1->B1_TIPO=="SU"            // Para MANUAL.
			AADD(xSERVICO,"N")
		Case SB1->B1_TIPO=="TM"            // Para Servi‡o (Insumos).
			AADD(xSERVICO,"S")
		Case SB1->B1_TIPO=="SW"            // Para Servi‡o (Software).
			AADD(xSERVICO,"S")
		Case SB1->B1_TIPO=="SE"            // Para Servi‡os Diversos.
			AADD(xSERVICO,"S")
		Case SB1->B1_TIPO=="RE"            // Para Grupo Construcao.
			AADD(xSERVICO,"N")  //20060911
		Case SB1->B1_TIPO=="CS"            // Para Volare.
			AADD(xSERVICO,"S")
		Case SB1->B1_TIPO=="PU"            // Para Publicidade PiniWeb.
			AADD(xSERVICO,"S")
		Case SB1->B1_TIPO=="GG"            // Para Despesas Diversas.
			AADD(xSERVICO,"S")
		Case SB1->B1_TIPO=="WW"            // Para Web.
			AADD(xSERVICO,"S")
		Case SB1->B1_TIPO=="MD"            // Para Despesa Agua.
			AADD(xSERVICO,"N")
		Case SB1->B1_TIPO=="RC"            // Para Relatorio de Custo 20030908
			AADD(xSERVICO,"S")		//20030908
	...
	For I := 1 to J
	    ...
	    AADD(xDESC_PRO,SB1->B1_DESC)   // 15/05/02 Alterado para buscar descricao em SB1(40 caracteres), pois em SC6 
	    If Alltrim(SC6->C6_PRODUTO) <> "0699001"
	    	AADD(xDESCRICAO,SB1->B1_DESC)  // o campo descricao tem tamanho de 30 e estava trucando na nota.
		Else
			AADD(xDESCRICAO,SC6->C6_DESCRI)			
		EndIf
...
//				XTOT_FAT := XTOT_FAT - (NVALIRRF) //comentado em 20040329 solicitado por Jose Ricardo Valenga
	...
	IncProc("Nota "+SF2->F2_SERIE+SF2->F2_DOC)                    // Termometro de Impressao
...
// Substituido pelo assistente de conversao do AP5 IDE em 30/03/02 ==> Function VerImp
...
@ _nLin01+1, 112 PSAY xNUM_NF             // Numero da Nota Fiscal.

/*ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ*/

@ _nLin02, 075 PSAY "X"
@ _nLin02+4, 002 PSAY "" //"NOVO END R.ANHAIA 964 BOM RETIRO/SP CEP 01130-900"         // 12/08/02
...
@ _nLin05,092  PSAY xCEP_CLI Picture"@R 99999-999"           // CEP
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
IMPMENP()    
//@ 59,002  PSAY "O REGISTRO DAS OPERACOES RELATIVAS A PRESTACAO DE SERVICOS, CONSTANTE DESTE DOCUMENTO, DEVERA SER CONVERTIDO POR NF-e" // 20060810 //20071010 desabilitado
@ 58,002  PSAY "PAGTO SOMENTE C/ BOLETO BANCARIO: NAO ACEITAMOS PAGTO VIA DOC, TRANSF. OU DEP. SIMPLES. POIS NOSSO SISTEMA NAO IDENTIFICA" // 20060522 //20071010 a linha era a 60
//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
// Substituido pelo assistente de conversao do AP5 IDE em 30/03/02 ==> Function DUPLIC
...
For I:=1 to nTamDet
	...
	If I <= Len(xCOD_PRO)
		...
		If (xSERVICO[I] #"S")
			...
			@ nLin, 016  PSAY SUBS(xDESCRICAO[I],1,40)//SUBS(xDESC_PRO[I],1,40) //"CD/MANUAL"     // descricao do produto.
			...
			@ nLin, 074  PSAY xQTD_PRO [I]   Picture"@E 999,999"        // quantidade do produto.
			...
			@ nLin, 118  PSAY xICM_PROD[I]   Picture"99"                // percentual do ICMS.
...
For I:= 1 to nTamDet
	...
	If I <= Len(xCOD_PRO)
		...
		If (xSERVICO[I]=="S")
			...
			@ nLin,002 PSAY AllTrim(xCOD_PRO [I])                     // Codigo do Produto.
			@ nLin,016 PSAY SUBS(xDESCRICAO[I],1,40)                  //SUBS(xDESC_PRO[I],1,40)  // Descricao do Produto no SC6.
			@ nLin,071 PSAY xUNID_PRO[I]                              // Unidade do Produto.
			@ nLin,074 PSAY xQTD_PRO [I]   Picture"@E 999,999"        // Quantidade do Produto.
			...
			@ nLin,120 PSAY xISS[I]		   Picture"99"                // Percentual do ISS (Preparar Sistema). //20031104
			_lImp := .T.
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
// Substituido pelo assistente de conversao do AP5 IDE em 30/03/02 ==> Function IMPMENP
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
// Substituido pelo assistente de conversao do AP5 IDE em 30/03/02 ==> Function MENSOBS
...
For BB := 8 to Len(xVALOR_DUP) // 20040813 era bb := 7
	IF  xPARC_DUP[BB] $ "H/I/J/K/L" // 20040813 era "G/H/I/J"
		@ nLin, 002  PSAY xNUM_DUP[BB]
		@ nLin, 010  PSAY xPARC_DUP[BB]
		@ nLin, 012  PSAY xVALOR_DUP[BB] Picture("@E 999,999.99")
		@ nLin, 025  PSAY xVENC_DUP[BB]
	ENDIF
	nLin := nLin + 1
Next
...
If xSup_Inf
	//          nlin:=nlin+1
	@ nLin, 002   PSAY "SUP.INF B.CALC ICMS DECR.45.490/00 ART.50 DO RICMS"
EndIf
// nlin:=nlin+1
...
IF BRETEM           
	IF MV_PAR04 = 1  //RETEM COFINS 
		...
		@ nLin, 058   PSAY NVALRETTOTAL Picture("@E 99,999.99")
	ENDIF
	IF MV_PAR05 = 1  //RETEM IR
		nlin:=nlin+1                                                              
...
ßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßßß
/*/
// Substituido pelo assistente de conversao do AP5 IDE em 30/03/02 ==> Function _ValidPerg
