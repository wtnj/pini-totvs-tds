...
//Danilo C S Pala 20060317: dados de enderecamento do DNE, etiqueta com 9 linhas, sendo a ultima ponto(.)
//Danilo C S Pala 20100305: ENDBP
User Function Rfat005()        // incluido pelo assistente de conversao do AP5 IDE em 25/02/02
...
*º Programa    : ETIQCP0.prx                                                 º
*º Descricao   : Etiquetas  para Pini Sistemas                               º
...
nLin:=0
MHORA := TIME()
wnrel    := "ETIQCP0_" + SUBS(CUSUARIO,7,3)+SUBS(MHORA,1,2)+SUBS(MHORA,7,2)
...
p_atu := 4
p_cnt := 0
//m_sav20 := dcursor(3) //20060320
//m_sav7 := savescreen(23,0,24,79) //20060320
***
...
//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
                ...
                *ÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ*
				...
				IF SM0->M0_CODIGO =="03" .AND. SA1->A1_ENDBP ="S"
					...
					CLIE_CEP  :=ZY3_CEP //ATE AQUI 20100305
			    ELSEIF SUBS(SA1->A1_ENDCOB,1,1)=='S'  .AND. SM0->M0_CODIGO <>"03"  //ATE AQUI 20100305
                        DbSelectArea("SZ5")
                        DbSetOrder(1)
                        DbSeek(XFilial()+XCLI+XLOJA)
                        CLIE_ENDE :=ALLTRIM(Z5_TPLOG)+ " " + ALLTRIM(Z5_LOGR) + " " + ALLTRIM(Z5_NLOGR) + " " + ALLTRIM(Z5_COMPL) //20060317
                        CLIE_BAIR :=Z5_BAIRRO
                        CLIE_MUNI :=Z5_CIDADE
                        CLIE_ESTA :=Z5_ESTADO
                        CLIE_CEP  :=Z5_CEP
                ELSE
                        CLIE_ENDE := ALLTRIM(SA1->A1_TPLOG) + " " + ALLTRIM(SA1->A1_LOGR) + " " + ALLTRIM(SA1->A1_NLOGR) + " " + ALLTRIM(SA1->A1_COMPL) //20060317
...
DBSELECTAREA("SZF")
                ...
                Reclock("SZF",.t.)
                REPLACE ZF_NF       WITH  XNFISCAL
                replaCE ZF_NOME     WITH  CLIE_NOME
                REPLACE ZF_RESPCOB  WITH  XRESPCOB
                replaCE ZF_END      WITH  CLIE_ENDE
                replaCE ZF_BAIRRO   WITH  CLIE_BAIR
                replaCE ZF_MUN      WITH  CLIE_MUNI
                replaCE ZF_EST      WITH  CLIE_ESTA
                replaCE ZF_CEP      WITH  CLIE_CEP
                SZF->(MSUnlock())
RETURN
...
SETPRC(0,0)
DO WHILE .NOT. EOF()
                @ LIN+LI,001 PSAY ZF_NOME
                DBSKIP()
                @ LIN+LI,045 PSAY ZF_NOME
                DBSKIP()
                @ LIN+LI,089 PSAY ZF_NOME
                DBSKIP(-2)
                LI:=LI+1

                @ LIN+LI,001 PSAY ZF_RESPCOB
                DBSKIP()
                @ LIN+LI,045 PSAY ZF_RESPCOB
                DBSKIP()
                @ LIN+LI,089 PSAY ZF_RESPCOB
                DBSKIP(-2)
                LI:=LI+1
 ...
 //20060317 ATE AQUI
                ...
                @ LIN+LI,001 PSAY ZF_BAIRRO
                DBSKIP()
                @ LIN+LI,045 PSAY ZF_BAIRRO
                DBSKIP()
                @ LIN+LI,089 PSAY ZF_BAIRRO
                DBSKIP(-2)
                LI:=LI+1

                @ LIN+LI,001 PSAY SUBS(ZF_CEP,1,5)+'-'+SUBS(ZF_CEP,6,3)+'   ' +ZF_MUN+' ' +ZF_EST
                DBSKIP()
                @ LIN+LI,045 PSAY SUBS(ZF_CEP,1,5)+'-'+SUBS(ZF_CEP,6,3)+'   ' +ZF_MUN+' ' +ZF_EST
                DBSKIP()
                @ LIN+LI,089 PSAY SUBS(ZF_CEP,1,5)+'-'+SUBS(ZF_CEP,6,3)+'   ' +ZF_MUN+' ' +ZF_EST
                LI:=LI+1                                                                         
