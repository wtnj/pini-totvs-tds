...
#include "rwmake.ch"
/*       ULTIMA ALTERACAO: 20031001
ÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜÜ
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณRFAT114   บAutor  ณDanilo C S Pala     บ Data ณ  09/29/03   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณIdentico ao IVC(Rfat024), exceto no intervalo de datas:     บฑฑ
ฑฑบ          ณ 20021231 e 20030301                                        บฑฑ
...
SetPrvt("nTCCapital,nTCInterior, nTCOutros, nTCTotal, cITEMPedido")
SetPrvt(" mVigenciaAss, mQtdPagto, mTipoop") //20031001
...
//mVigenciaAss:  refere-se a assinatura ser Anual ou bineal
//mQtdPagto :  qtd de parcelas a ser paga a assinatura
//mTipoop : tipo da operacao, para determinas a mqtdpagto
CLIE_DEST:=SPACE(40)
...
Processa({|lEnd| R024Proc(@lEnd)}, "IVC Auditoria", "Aguarde, Processando IVC...")
...
Private mHora       := TIME()
Private wnrel       := "DIVCAuditoria_" + SUBS(CUSUARIO,7,3)+SUBS(MHORA,1,2)+SUBS(MHORA,7,2)
Private nLastKey    := 0
...
//_cFiltro := _cFiltro+".and.MV_PAR02=C6_EDINIC .AND. MV_PAR02<=C6_EDVENC" //20031118
...
WHILE !Eof() .and. SC6->C6_FILIAL == xFilial("SC6") .and. MV_PAR01 == SC6->C6_REGCOT
	...
	//20030716			
	IF (SubStr(SC6->C6_PRODUTO, 5,3) = "001")
		DbSelectArea("SC6")
		DbSkip()
		Loop
	ENDIF 
	...
	cITEMPedido := SC6->C6_ITEM //20030716
...
/*//teste 20031001
	...
	//20031001 vigencia da assinatura
	if substr(mProduto,5,3) = "001"
		mVigenciaAss := "AVULSA" // NAO ENTRA NO IVC
	elseif substr(mProduto,5,3) = "002"
		mVigenciaAss := "NOVA ANUAL"
	elseif substr(mProduto,5,3) = "003"
		mVigenciaAss := "NOVA BIENAL"
	elseif substr(mProduto,5,3) = "004"
		mVigenciaAss := "RENOV ANUAL"
	elseif substr(mProduto,5,3) = "005"
		mVigenciaAss := "RENOV BIENAL"
	else
		mVigenciaAss := "OUTRA"
	endif
...
//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	...
	MEND      := ALLTRIM(SA1->A1_TPLOG) + " " + ALLTRIM(SA1->A1_LOGR) + " " + ALLTRIM(SA1->A1_NLOGR) + " " + ALLTRIM(SA1->A1_COMPL) //20060323
	...
	//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤ ฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
	//ณPosiciona Pedido e get o tipoopณ
	//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤ ฤฤฤฤฤฤฤฤฤฤฤฤฤู
	DBSELECTAREA("SC5")
	DBSETORDER(1)
	...
	IF MGRAT=='S'.OR. SC6->C6_ORIGEM=='ED'
		MEDSUSP:=SC6->C6_EDSUSP
//20031001		IF SC6->C6_EDSUSP#0 .AND. SC6->C6_EDSUSP<=MV_PAR02 .AND. SC6->C6_EDSUSP<SC6->C6_EDVENC
		IF (SC6->C6_EDSUSP#0) .AND. (SC6->C6_EDSUSP < SC6->C6_EDVENC) //cancelada?
			GR_TEMP2()
			DBSELECTAREA("SC6")
			DBSKIP()
			LOOP
	...
	IF SC6->C6_SITUAC=='CP' .OR. SC6->C6_SITUAC=='SE' .OR. SC6->C6_SITUAC=='SU' .OR. SC6->C6_SITUAC=='SC';
		.OR. SC6->C6_SITUAC='LP' .OR. SC6->C6_SITUAC='CA' .OR. SC6->C6_SITUAC='SI'
		MEDSUSP:=SC6->C6_EDSUSP
// 20031001		IF SC6->C6_EDSUSP#0 .AND. SC6->C6_EDSUSP<=MV_PAR02.AND. SC6->C6_EDSUSP<SC6->C6_EDVENC
		IF (SC6->C6_EDSUSP#0) .AND. (SC6->C6_EDSUSP < SC6->C6_EDVENC) //cancelada?
			GR_TEMP2()
	...
	IF !DBSEEK(XFILIAL()+MPEDIDO)
		MNAO    := 'NAO'
		MEDSUSP := SC6->C6_EDSUSP
//20031002		IF SC6->C6_EDSUSP#0 .AND. SC6->C6_EDSUSP<=MV_PAR02 .AND. SC6->C6_EDSUSP<SC6->C6_EDVENC
		IF (SC6->C6_EDSUSP#0) .AND. (SC6->C6_EDSUSP < SC6->C6_EDVENC) //cancelada?
			GR_TEMP2()
			DBSELECTAREA("SC6")
			DBSKIP()
...
COPY TO &_StrArqAVERIG VIA "DBFCDXADS" // 20121106 
DBCLOSEAREA()            
...
IF SA1->A1_ATIVIDA >= MV_PAR06 .AND. SA1->A1_ATIVIDA <= MV_PAR07 
 IF (DTOS(SC6->C6_DATA) > '20021231' .AND. DTOS(SC6->C6_DATA) < '20030301') .AND. Mgrat <> "S" //diferenca do rfat024
	RETURN
 ELSE 
	DBSELECTAREA("ETIQP")
	Reclock("ETIQP",.t.)
...
//20030707
IF SA1->A1_ATIVIDA >= MV_PAR06 .AND. SA1->A1_ATIVIDA <= MV_PAR07 //.AND. DTOS(SC6->C6_DATA) >= '20030301' //20030929
	DBSELECTAREA("COMPLP")
	Reclock("COMPLP",.t.)
	REPLACE NUMPED  WITH  MPEDIDO
	REPLACE ITEM    WITH  MITEM
...
replace ITEM  WITH cITEMPedido //20030716
...
//20030929
REPLACE MUN WITH MCIDADE
REPLACE ESTADO WITH MEST
//20031001
REPLACE VIGENCIA WITH MVIGENCIAASS
REPLACE QTDPAGTO WITH MQTDPAGTO
MsUnlock("AVERIG")
RETURN