...
±±³ Programa ³ PFAT117  ³ Autor ³ Gilberto A Oliveira ³ Data ³Seg  21/07/00³±±
...
±±³Uso       ³ Espec¡fico para Pini Sistemas Ltda.                         ³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ´±±
...
/*/
User Function Pfat117()        // incluido pelo assistente de conversao do AP5 IDE em 26/02/02
...
SetPrvt("_CMSG,_AREJEITADOS,_LBOLETO,_LSAIDA,LHEADER,_CSERIE")
SetPrvt("_NHDL,_CLIN,_LOK,_NSEQ,_CINCREMENTA,HEAD_001_A_001")
SetPrvt("HEAD_002_A_002,HEAD_003_A_009,HEAD_010_A_011,HEAD_012_A_026,HEAD_027_A_037,HEAD_038_A_046")
SetPrvt("HEAD_047_A_076,HEAD_077_A_079,HEAD_080_A_086,HEAD_087_A_094,HEAD_095_A_100,HEAD_101_A_105")
SetPrvt("HEAD_106_A_108,HEAD_109_A_117,MV_PAR10,_CMULTA,_CMULTA1,_CPRAZO,_CDESC")
SetPrvt("_CXMSG0,_CXMSG1,_CXMSG2,_CXMSG3,HEAD_118_A_184,HEAD_185_A_252")
SetPrvt("HEAD_253_A_319,HEAD_320_A_386,HEAD_387_A_394,HEAD_395_A_400,_AHEADER,_N1")
SetPrvt("MV_PAR09,_CMSGAVISO,_CMSGERROR,_CCEP,_LOKFAIXA,_NFAIXAATU")
...
SetPrvt("WNREL,LI,ADRIVER,NTIPO,NLIN,")
...
±±ºPrograma  ³RUNPROC   ºAutor  ³Microsiga           º Data ³  03/30/02   º±±
...
If Alltrim(mv_par01) <> "033"
	_cMsg1:= "O Banco para essa rotina deve ser o 033 (Banespa). Verifique !!!"
	_lContinua:= .F.
Endif
...
DbSelectArea("SC6")
DbSetOrder(4)
DbSeek(xFilial("SC6")+MV_PAR04+MV_PAR06, .T.)   // Nota Fiscal Inicial.
If SC6->C6_NOTA > MV_PAR05 .Or. SC6->C6_SERIE <> MV_PAR06
	_cMsg:= "Nota Fiscal nao Encontrada no arquivo de pedidos !!! Verifique..."
...
DbSelectArea("SF2")
DbSetOrder(1)
DbSeek(xFilial("SF2")+mv_par04+mv_par06,.T.)
If SF2->F2_DOC > MV_PAR06    //.OR. SF2->F2_SERIE #MV_PAR07
	MsgAlert("Nao foram encontrados Notas Fiscais para Impressao !! Verifique ...","ATENCAO")
	Return
EndIf
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³RUNCONT   ºAutor  ³Microsiga           º Data ³  03/30/02   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
While !EOF() .and. SF2->F2_FILIAL == xFilial("SF2") .AND. SF2->F2_DOC <= MV_PAR05
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³ Incrementa a regua                                                  ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	IncProc("Processando Titulo "+Alltrim(SF2->F2_SERIE+SF2->F2_DOC))
	
	//ÚÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄ¿
	//³ Identifica a serie da nota                                   ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ
	If SF2->F2_SERIE # MV_PAR06           // Se a Serie do Arquivo for Diferente
	...
	If !DbSeek(xFilial("SE1")+_cSerie+SF2->F2_DOC)
		_cMsg:= "Duplicata nao Encontrada !!!"
...
//		Head_047_a_076 := SubStr(SM0->M0_NOMECOM,1,30)  
		Head_047_a_076 := "PINI SISTEMAS LTDA"
...
//	  _cXMsg2 := iif(mv_par07  > 0 , "MAIS R$ "+_cMulta+" POR DIA DE ATRASO." , Space(2) )
	...
	While !Eof() .And. SE1->E1_FILIAL+SE1->E1_NUM==xFilial("SE1")+SF2->F2_DOC
		...
		IF SE1->E1_SITUACA # "0"
			...
			DbSelectArea("SE1")
			DbSkip()               // retirado provisoriamente
			Loop                   // voltar novamente.
		ENDIF
...
Else
	...
	_cMsgINFO:= _cMsgINFO + "Anote o nome do Arquivo : "+_cPathBanespa+_cArquivo
	MsgInfo(OemToAnsi(_cMsgInfo))
	WinExec(_cPathBol+_cPathBanespa+"VBISLIP N "+_cArquivo)
EndIf
...
If !MsgYesNo("Ocorreu um erro na grava‡„o do arquivo "+_cArquivo+".   Continua?","Aten‡„o!")
	_lOk:= .F.
Endif
Return
/*/
...
DbSelectArea("SEE")
DbSetOrder(1)
If DbSeek(xFilial("SEE")+ _cNBco + _cAgen + _cCCor )
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³CalculaDigºAutor  ³Microsiga           º Data ³  03/30/02   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³Calculo do Digito Verificador.                              º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ AP5                                                        º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³GANHATEMPOºAutor  ³Microsiga           º Data ³  03/30/02   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³                                                            º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÉÍÍÍÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍËÍÍÍÍÍÍÑÍÍÍÍÍÍÍÍÍÍÍÍÍ»±±
±±ºPrograma  ³PFAT117   ºAutor  ³Microsiga           º Data ³  03/30/02   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDesc.     ³VERFAIXACEPS                                                º±±
±±º          ³                                                            º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ AP5                                                        º±±
±±ÈÍÍÍÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¼±±
...
// Substituido pelo assistente de conversao do AP5 IDE em 26/02/02 ==> Function ImpDel
...
m_pag := 0
MHORA      := TIME()
wnrel := "PFAT117_" + SUBS(CUSUARIO,7,3)+SUBS(MHORA,1,2)+SUBS(MHORA,7,2)
