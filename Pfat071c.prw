...
╠╠ЁDescricao Ё Nota Fiscal de Saida.                                         Ё╠╠
...
/*/
User Function Pfat071c()        // incluido pelo assistente de conversao do AP5 IDE em 27/03/02
...
//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
RptStatus({|| RptDetail()})// Substituido pelo assistente de conversao do AP5 IDE em 27/03/02 ==>     RptStatus({|| Execute(RptDetail)})
...
//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
IMPMENP()
//@ 59,002  PSAY "O REGISTRO DAS OPERACOES RELATIVAS A PRESTACAO DE SERVICOS, CONSTANTE DESTE DOCUMENTO, DEVERA SER CONVERTIDO POR NF-e" // 20060810
@ 60,002  PSAY "PAGTO SOMENTE C/ BOLETO BANCARIO: NAO ACEITAMOS PAGTO VIA DOC, TRANSF. OU DEP. SIMPLES. POIS NOSSO SISTEMA NAO IDENTIFICA"  // 20060522
