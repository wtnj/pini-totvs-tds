...
aAdd(aC,{"m->F1_VALBRUT"  ,{20,250},"Valor Nota"         ,"@E 9,999,999.99" , ,     ,.F.})   
mCusto
aCGD :=  {50,5,108,280}
...
if lRet
	if MsgYesNo("Confirma os Dados Digitados?",cTitulo)
		//Processa({||U_GravaSDE(cAlias)},cTitulo,"Gravando as informações, aguarde por favor!!")
		GravaSDE(cAlias)
	endif
else
	rollbackSX8()
...
if mCustoNFE <> nCusto1 
	MsgInfo("A somatório dos valores do rateio é diferente do valor total da Nota!!!",cTitulo)
	lRet := .f.
endif                              
if nDel == Len(aCols)
	MsgInfo("Nesta Opção não é possível excluir dados!!!",cTitulo)
	lRet := .f.
endif
Return lRet
