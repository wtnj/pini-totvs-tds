...
╠╠ЁPrograma: PFAT146   ЁAutor: Raquel Ramalho         Ё Data:   13/12/01 Ё ╠╠
╠╠цддддддддддддддддддддаддддддддддддддддддддддддддддддадддддддддддддддддд╢ ╠╠
╠╠ЁDescri┤ao: Gera arquivo DBF dos Recebimentos                          Ё ╠╠
...
//Ё mv_par01 Vencimento de:                                                 Ё
//Ё mv_par02 Vencimento at┌:                                                Ё
//Ё mv_par03 Baixado   Em Aberto    Ambos                                   Ё
//| MV_PAR04 VENCIMENTO EMISSAO												Ё
//Ё mv_par05 cliente de 				                                    Ё
//Ё mv_par06 cliente ate 				                                    Ё
...
//Ё Caso nao exista, cria grupo de perguntas.                    Ё
...
//Ё Function  Ё Filtra()                                                      Ё
//цдддддддддддеддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
//Ё Descricao Ё Filtra arquivo SE1 para ser utilizado no programa.            Ё
...
Return
//здддддддддддбддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё Function  Ё GRAVA()                                                       Ё
//цдддддддддддеддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
//Ё Descricao Ё Realiza gravacao dos registros ideais (conforme parametros)   Ё
//Ё           Ё para impressao de Relatorio.                                  Ё
//цдддддддддддеддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
//Ё Observ.   Ё                                                               Ё
//юдддддддддддаддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
...
Return
//здддддддддддбддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё Function  Ё FARQTRAB()                                                    Ё
//цдддддддддддеддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
//Ё Descricao Ё Cria arquivo de trabalho para guardar registros que serao     Ё
//Ё           Ё impressos em forma de etiquetas.                              Ё
//Ё           Ё serem gravados. Faz chamada a funcao GRAVA.                   Ё
//цдддддддддддеддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢
//Ё Observ.   Ё                                                               Ё
//юдддддддддддаддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
