...
±±ºPrograma  ³PEDSITEC1 º Autor ³ Douglas Silva      º Data ³  31/03/14   º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÊÍÍÍÍÍÍÏÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºDescricao ³ Este fonte tem como objetivo importar e tratar os pedidos  º±±
±±º          ³ gerados pela contentstaff                                  º±±
±±ÌÍÍÍÍÍÍÍÍÍÍØÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍÍ¹±±
±±ºUso       ³ AP6 FATURAMENTO                                            º±±
...
DEFINE MSDIALOG oDlg TITLE "Controle de Importação de Pedido de Venda" FROM 000, 000  TO 150, 400 COLORS 0, 16777215 PIXEL
...
////////////////////////////////////////////////////////////////////////////////////////////////////
// Criação das cores para controle                                                                //
...
AADD(aCores,{"ZC1_STATUS == '2'" ,"BR_AMARELO" 	}) //Pedido de Venda não processado
...
//Montagem da legenda das ocorrências da importação
...
PRIVATE lMSErroAuto := .f. 
  ...
  DEFINE MSDIALOG oDlg TITLE "Controle de Importação ContentStaff" FROM 000, 000  TO 200, 500 COLORS 0, 16777215 PIXEL
		...
		If !empty(cLinha )
			...
			If Substr(cLinha,1,1 )=="2"    
				...
				aadd(aCabec,	{	Substr(cLinha,2,8	),;		//1Numero do Pedido
									...
									Substr(cLinha,190,50),; 	//14Endereço
			...
			ElseIf Substr(cLinha,1,1 )=="5"
				...
				aadd(aTrailer,{		Substr(cLinha,2,8 	),;		//1Numero do Pedido
						            ...
						            Substr(cLinha,34,10 ),;     //5Situação da Parcela
...
'
		If ! SZ0->(DBSEEK(xFilial("SZ0") + ZC1->ZC1_CEP ))  
			If MSGYESNO("CEP do Cliente " +  SUBSTR(ZC1->ZC1_CEP,1,5) + "-" + SUBSTR(ZC1->ZC1_CEP,6,3)  + " não localizado na tabela dos correios, deseja continuar?")
				MsgAlert("Cliente bloqueado favor revisar as informações do mesmo!")
				_cBloq := "2"
			Else
				Return
			EndIf
...
'                                                                 
		...
		If ! CC2->(DBSEEK(xFilial("CC2") + _cEstado + _cCodMun ))  
			MsgAlert("Codigo de Munincipio : " + _cCodMun + " Não localizado, favor verificar a tela de Endereço!")
			Return
		EndIf		
           ...
           //_cEnd,_cBairro,_cCep,_cEstado,_cMunip,_cCodMun,_cComplem
			aVetor:={	{"A1_COD"       	,cNumCod		       	,Nil},; // Codigo				 
						...
						//+---------------------------------------------------------------------+
						//| Se houve erro na gração do novo cliente é mostrado na tela          |
						//+---------------------------------------------------------------------+
...
Private _nDescP := 0
	...
	//+---------------------------------------------------------------------+
	//| Verifica se codigo do cliente está preenchido						|
	//+---------------------------------------------------------------------+	 
	...
	If ! EMPTY(ZC1->ZC1_CLIEPR)
		...
		If EMPTY(ZC1->ZC1_PEDIDO)
			...
			//+---------------------------------------------------------------------+
			//| Query para verificar se já existe o pedido nas tabelas SC5 OU SC6   |
			//+---------------------------------------------------------------------+	 
			...
			Else   
				...
				//+---------------------------------------------------------------------+
				//| Verifica se pedido já está no protheus                              |
				//+---------------------------------------------------------------------+	 
			...
			EndIf
		Else
			MsgInfo("Pedido de Venda Site já esta no Protheus: " + ZC1->ZC1_PEDIDO)
	...
	Else
		...
		MsgAlert("Cliente não vinculado ao pedido de venda site, execute primeiro a integração cliente x pedido site!") 
		Return
	...
	Endif
		...
		If EMPTY(TC2->ZC2_PEDSIT)
			MsgAlert("Pedido de venda com problema, não encontrado detalhe do pedido: " + ZC1->ZC1_PEDSIT) 
			RECLOCK("ZC1",.F.)
				ZC1->ZC1_STATUS := "2"
			ZC1->(MSUNLOCK())
			TZ2->(DBCLOSEAREA("TC2"))  
		...
		Else
			...
			Do While TC2->(!EOF())
				...
				If ! TC2->B1_TIPO $ "LI|LC|LD|CD|RE" 
					...
					MsgAlert("Produto " + TC2->B1_COD + " Decrição : " + TC2->B1_DESC + " Não Pertecem a Livros!")
					RECLOCK("ZC1",.F.)
						ZC1->ZC1_STATUS := "2"
					ZC1->(MSUNLOCK())
					TZ2->(DBCLOSEAREA("TC2"))
		...
		//+---------------------------------------------------------------------+
		//| Verifica se já Existe Pedido de Venda                               |
		//+---------------------------------------------------------------------+	 		
		...
		If ! SC5->(DBSEEK(xFilial("SC5") + cNumPed )) 
				...
				RECLOCK("SC5",.T.)
					...
					//+---------------------------------------------------------------------+
					//| Gera as Informçaões de Pagamento                                    |
					//+---------------------------------------------------------------------+	 
					
					SC5->C5_DTCALC	:= ddatabase 
					SC5->C5_TIPOOP	:= U_PEDSITPG()Í
			...
			Do While TC2->(!EOF())
				...
				If SB1->(DBSEEK(xFilial("SB1") + TC2->ZC2_CODTSI ))
					...
					Else
						...
						//+---------------------------------------------------------------------+
						//| Caso o produto não tenha TES ou não está valida na SF4              |
						//+---------------------------------------------------------------------+
				...
				Else
					...
					Alert("Produto não cadastrado Pedido Site: " + ALLTRIM(ZC1->ZC1_PEDSIT) + " " + ALLTRIM(TC2->ZC2_CODTSI) + " " + " Descrição " + " " + ALLTRIM(TC2->ZC2_DESCS) )
		...
		Else
			...
			MsgAlert("Numeração do pedido de venda com erro: " + UPPER(cNumPed))
		 	Return
		EndIf
...
cQuery := " SELECT CC2_CODMUN FROM "+RETSQLNAME("CC2")+" WHERE D_E_L_E_T_ != '*' AND CC2_EST = '"+_cEstado+"' AND (TRANSLATE((UPPER(CC2_MUN)),'âàãáÁÂÀÃéêÉÊíÍóôõÓÔÕüúÜÚÇç','AAAAAAAAEEEEIIOOOOOOUUUUCC')) LIKE '%"+_cMunip+"%' "
...
If ! SZ0->(DBSEEK(xFilial("SZ0") + ZC1->ZC1_CEP ))  
	MsgAlert("CEP do Cliente " +  SUBSTR(ZC1->ZC1_CEP,1,5) + "-" + SUBSTR(ZC1->ZC1_CEP,6,3)  + " não localizado na tabela dos correios, deseja continuar?")
EndIf		

	DEFINE MSDIALOG oDlg TITLE "Endereço Cliente/Entrega" FROM 000, 000  TO 245, 600 COLORS 0, 16777215 PIXEL
	
    @ 003, 002 GROUP oGroup1 TO 120, 300 PROMPT OF oDlg COLOR 0, 16777215 PIXEL
    
    @ 013, 007 SAY oSay7 PROMPT "Código" 		SIZE 025, 007 OF oDlg COLORS 0, 16777215 PIXEL
...
If SA1->(DBSEEK(xFilial("SA1") + _cCPFCGC ))		
  ...
  DEFINE MSDIALOG oDlg TITLE "Alterar Endereço Cliente/Entrega" FROM 000, 000  TO 470, 600 COLORS 0, 16777215 PIXEL
