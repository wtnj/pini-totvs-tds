...
AADD(aCores,{"ZC1_STATUS == '1'" ,"BR_AMARELO" 	}) //Pedido de Venda não processado
...
AADD(aLegenda,{"BR_AMARELO"	,"Pedido de Venda não processado"	}) 
...
_cCPFCGC := IIF(aCabec[1][11] = "J", aCabec[1][5],SUBSTR(aCabec[1][5],4,11))
	...
	If ! SA1->(DBSEEK(xFilial("SA1") + _cCPFCGC ))
		...
		If ! SZ0->(DBSEEK(xFilial("SZ0") + cValToChar(aCabec[1][13]) ))  
			U_MailNotify(_cMail,,"IMPORTAÇÃO PEDIDO DE VENDA EDITORA PINI ERRO CEP ",{"CEP NÃO CADASTRADO NA BASE DE PARA CORREIOS " + Time() + " --- " + Memoread( "Cliente: " + cNumCod + " CEP " + SUBSTR(aCabec[1][13],1,5) + "-" + SUBSTR(aCabec[1][13],6,3)) },,.F.)	
			_cBloq := "2"
		EndIf		
			...
			aVetor:={	{"A1_COD"       	,cNumCod		       	,Nil},; // Codigo				 
						...
						{"A1_INSCR"   		,aCabec[1][29]			,Nil},; // Incrição Estadual
	...
	Else
		...
		If ! SA1->A1_CEP != aCabec[1][13]
			U_MailNotify(_cMail,,"IMPORTAÇÃO PEDIDO DE VENDA EDITORA PINI CEP ",{"CEP DIFERENTE DA BASE PROTHEUS X PEDIDO TXT " + Time() + " --- " + Memoread( "Cliente: " + SA1->A1_COD + " CEP TXT " + SUBSTR(aCabec[1][13],1,5) + "-" + SUBSTR(aCabec[1][13],6,3) + " CEP PROTHEUS " + SUBSTR(SA1->A1_CEP,1,5) + "-" + SUBSTR(SA1->A1_CEP,6,3)  ) },,.F.)	
			_cBloq := "2"
		EndIf
		...
		If ! SC5->(DBSEEK(xFilial("SC5")+ cNumPed )) 
			...
			For n := 1 To len(aItens1)
				...
				Else
					...
					Alert("Produto não cadastrado Pedido Site: " + ALLTRIM(aCabec[1][1]) + " " + ALLTRIM(aItens1[N][3]) + " " + " Descrição " + " " + ALLTRIM(aItens1[N][4]) )
