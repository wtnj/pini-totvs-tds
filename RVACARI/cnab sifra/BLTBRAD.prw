...
//NOVO-01-12-2013 - THIAGO

IF MV_PAR22 <> '   '
	
    IF MSGYESNO("Confirma atualizacao de dados para novo portador?","Atualizacao de dados")
	...
	//³ Busca Títulos e grava novo portador  ³
	//ÀÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÙ

	cQuery := " SELECT E1_PREFIXO, E1_SERIE, E1_NUM, E1_PARCELA, E1_PORTADO, E1_AGEDEP, E1_CONTA, E1_SITUACA"
	...
	cQuery += " AND E1_NATUREZ BETWEEN '"+MV_PAR26+"' AND '"+MV_PAR27+"'  "
...
//	dbSelectArea("TITULOS")
	...
	//MSGALERT(QRY->E1_NUM+" - "+QRY->E1_PARCELA) 

	dbSelectArea("SE1")
	dbSetOrder(30)
   
	IF SE1->(dbSeek(xFilial() + QRY->E1_NUM + QRY->E1_SERIE + QRY->E1_PARCELA,.T.) ) 
		
		RecLock("SE1",.F.)
			
			SE1->E1_PORTADO := MV_PAR22
			SE1->E1_AGEDEP  := MV_PAR23
			SE1->E1_CONTA   := MV_PAR24
		//	SE1->E1_SITUACA := MV_PAR25    
	else
...
//	dbSelectArea("TITULOS")
	...
	EndDo                               
		...
		dbCloseArea("QRY")
		endif //fecha o segundo if que teste o MSGYESNO()
endif //fecha o primeiro if que testa o MVPAR22 <> 0
...
LOCAL aBolText     := {"Após o vencimento cobrar multa de " + AllTrim(Str(mv_par20)) + " % = R$ "                ,;
					"Mora Diaria de R$ "                                   ,;
					"Não aceitamos pagamento via DOC, Transferência ou Depósito"                                   ,;
					"Sujeito a Protesto apos " + AllTrim(Str(mv_par21)) + " dias do vencimento",;
					"DÚVIDAS, 2ª VIA DE BOLETO, ATUALIZAÇÃO OU PRORROGAÇÃO LIGUE PARA (11) 2173-2445",;
...
Do While !EOF()
	...
	aDadosBanco  := {SA6->A6_COD                        ,; 							// [1]Numero do Banco
                      ...
                      SUBSTR(SA6->A6_DVCTA,Len(AllTrim(SA6->A6_DVCTA)),1)  ,; 	// [5]Dígito da conta corrente
...
//	If Empty(SA1->A1_ENDCOB)
	...
	Else      
		...
		aDatSacado   := {AllTrim(SA1->A1_NOME)            	 ,;   	// [1]Razão Social
		AllTrim(SA1->A1_COD )+"-"+SA1->A1_LOJA              ,;   	// [2]Código
		AllTrim(SZ5->Z5_END)+"-"+AllTrim(SZ5->Z5_BAIRRO),;   	// [3]Endereço
		AllTrim(SZ5->Z5_CIDADE)	                             ,;   	// [4]Cidade
		SZ5->Z5_ESTADO	                                     ,;   	// [5]Estado
		SZ5->Z5_CEP                                       ,;   	// [6]CEP
		SA1->A1_CGC												 		 ,;		// [7]CGC
		SA1->A1_PESSOA												 }				// [8]PESSOA
	Endif
...
//oPrint:Line (nRow1+0150,500,nRow1+0070, 500)    thiago
//oPrint:Line (nRow1+0150,710,nRow1+0070, 710)    thiago

oPrint:Line (nRow1+0150,600,nRow1+0070, 600)
oPrint:Line (nRow1+0150,800,nRow1+0070, 800)
...
oPrint:Say  (nRow1+0075,615,aDadosBanco[1]+"-2",oFont21 )		// [1]Numero do Banco
...
oPrint:Line (nRow2+0710,600,nRow2+0630, 600)
oPrint:Line (nRow2+0710,800,nRow2+0630, 800)

oPrint:Say  (nRow2+0644,100,aDadosBanco[2],oFont14 )		// [2]Nome do Banco
oPrint:Say  (nRow2+0635,615,aDadosBanco[1]+"-2",oFont21 )	// [1]Numero do Banco
...
//cString := Alltrim(Substr(aDadosTit[6],1,3)+"/"+Substr(aDadosTit[6],4))        THIAGO
cString :=   "19 / "+NOSSONUM+"-"+DIGITOVER
...
oPrint:Say  (nRow2+1090,100 ,aBolText[1]+" "+AllTrim(Transform((aDadosTit[5] * (mv_par20 /100)),"@E 99,999.99"))+" - "+aBolText[2]+" "+AllTrim(Transform((aDadosTit[5] * ((mv_par19 /30)/100)),"@E 99,999.99"))     ,oFont10) 
//oPrint:Say  (nRow2+1140,100 ,aBolText[2]+" "+AllTrim(Transform((aDadosTit[5] * ((mv_par19 /30)/100)),"@E 99,999.99"))  ,oFont10)
oPrint:Say  (nRow2+1140,100 ,aBolText[3]                                        ,oFont10)
oPrint:Say  (nRow2+1190,100 ,aBolText[4]                                        ,oFont10)
oPrint:Say  (nRow2+1290,100 ,aBolText[5]                                        ,oFont10)

	if (MV_PAR28 >0)
oPrint:Say  (nRow2+1240,100 ,aBolText[6]+" "+AllTrim(Transform(mv_par28,"@E 99,999.99")) ,oFont10)
	endif
...
oPrint:Line (nRow3+2000,600,nRow3+1920, 600)
oPrint:Line (nRow3+2000,800,nRow3+1920, 800)

oPrint:Say  (nRow3+1934,100,aDadosBanco[2],oFont14 )		// 	[2]Nome do Banco
oPrint:Say  (nRow3+1925,615,aDadosBanco[1]+"-2",oFont21 )	// 	[1]Numero do Banco
oPrint:Say  (nRow3+1934,810,aCB_RN_NN[2],oFont15n)			//		Linha Digitavel do Codigo de Barras
...
oPrint:Say  (nRow3+2390,100 ,aBolText[1]+" "+AllTrim(Transform((aDadosTit[5] * (mv_par20 /100)),"@E 99,999.99"))+" - "+aBolText[2]+" "+AllTrim(Transform((aDadosTit[5] * ((mv_par19 /30)/100)),"@E 99,999.99"))       ,oFont10)
//oPrint:Say  (nRow3+2440,100 ,aBolText[2]+" "+AllTrim(Transform((aDadosTit[5] * ((mv_par19 /30)/100)),"@E 99,999.99"))  ,oFont10)
oPrint:Say  (nRow3+2440,100 ,aBolText[3]                                        ,oFont10)
oPrint:Say  (nRow3+2490,100 ,aBolText[4]                                        ,oFont10)
oPrint:Say  (nRow3+2590,100 ,aBolText[5]                                        ,oFont10)

	if (MV_PAR28 >0)
oPrint:Say  (nRow2+2540,100 ,aBolText[6]+" "+AllTrim(Transform(mv_par28,"@E 99,999.99")) ,oFont10)
	endif
	
//oPrint:Say  (nRow3+1150,100 ,aBolText[1]+" "+AllTrim(Transform((aDadosTit[5] * (mv_par20 /100)),"@E 99,999.99"))       ,oFont10)
//oPrint:Say  (nRow3+1200,100 ,aBolText[2]+" "+AllTrim(Transform((aDadosTit[5] * ((mv_par19 /30)/100)),"@E 99,999.99"))  ,oFont10)
//oPrint:Say  (nRow3+1250,100 ,aBolText[3]                                        ,oFont10)

//oPrint:Say  (nRow3+2590,100 ,aCB_RN_NN[1]                                       ,oFont10) // CODIBAR
oPrint:Say  (nRow3+2640,100 ,aCB_RN_NN[1]                                       ,oFont10) // CODIBAR
...
IF MV_PAR28 > 0
	...
	DbSelectArea("SE1") 
	RecLock("SE1",.f.)  
	SE1->E1_OBS := "Desconto concedido de R$ "+CVALTOCHAR(MV_PAR28) 
	MsUnlock()                                                                            
...
cS    :=  cCart + cNroDoc //19 001000012     THIAGO
//nDvnn := modulo11(cS) // digito verifacador         THIAGO
//cNNSD := cS //Nosso Numero sem digito                     THIAGO
//cNN   := cCart + cNroDoc + '-' + AllTrim(Str(nDvnn))            THIAGO
...
nDvnn 	 := SUBSTR(SE1->E1_NUMBCO,12,1)            
cNNSD 	 := SUBSTR(SE1->E1_NUMBCO,1,11)
cNN   	 := SE1->E1_NUMBCO
NOSSONUM := cNNSD
DIGITOVER:= nDvnn            
...
PutSx1(cPerg,"02","Prefixo até         "  , "", "", "mv_ch2","C",03,0,0,"G","", "", "", "", "MV_PAR02",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"03","NF de               "  , "", "", "mv_ch3","C",09,0,0,"G","", "", "", "", "MV_PAR03",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"04","NF até              "  , "", "", "mv_ch4","C",09,0,0,"G","", "", "", "", "MV_PAR04",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"05","Parcela de          "  , "", "", "mv_ch5","C",01,0,0,"G","", "", "", "", "MV_PAR05",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"06","Parcela até         "  , "", "", "mv_ch6","C",01,0,0,"G","", "", "", "", "MV_PAR06",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"07","Portador de         "  , "", "", "mv_ch7","C",03,0,0,"G","", "", "", "", "MV_PAR07",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"08","Portador até        "  , "", "", "mv_ch8","C",03,0,0,"G","", "", "", "", "MV_PAR08",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"09","Cliente de          "  , "", "", "mv_ch9","C",06,0,0,"G","", "", "", "", "MV_PAR09",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"10","Cliente até         "  , "", "", "mv_cha","C",06,0,0,"G","", "", "", "", "MV_PAR10",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"11","Loja de             "  , "", "", "mv_chb","C",02,0,0,"G","", "", "", "", "MV_PAR11",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"12","Loja até            "  , "", "", "mv_chc","C",02,0,0,"G","", "", "", "", "MV_PAR12",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"13","Emissão de          "  , "", "", "mv_chd","D",08,0,0,"G","", "", "", "", "MV_PAR13",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"14","Emissão até         "  , "", "", "mv_che","D",08,0,0,"G","", "", "", "", "MV_PAR14",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"15","Vencimento de       "  , "", "", "mv_chf","D",08,0,0,"G","", "", "", "", "MV_PAR15",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"16","Vencimento até      "  , "", "", "mv_chg","D",08,0,0,"G","", "", "", "", "MV_PAR16",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"17","Borderô de          "  , "", "", "mv_chh","C",06,0,0,"G","", "", "", "", "MV_PAR17",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"18","Borderô até         "  , "", "", "mv_chi","C",06,0,0,"G","", "", "", "", "MV_PAR18",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"19","Jurus Dia Atraso    "  , "", "", "mv_chj","N",11,2,0,"G","", "", "", "", "MV_PAR19",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"20","Multa               "  , "", "", "mv_chk","N",11,2,0,"G","", "", "", "", "MV_PAR20",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"21","Dias Protesto       "  , "", "", "mv_chl","N",02,2,0,"G","", "", "", "", "MV_PAR21",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
//NOVO 01-12-2013- THIAGO
PutSx1(cPerg,"22","Novo Portador       "  , "", "", "mv_chm","C",03,0,0,"G","", "", "", "", "MV_PAR22",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"23","Nova Agencia        "  , "", "", "mv_chn","C",05,0,0,"G","", "", "", "", "MV_PAR23",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"24","Nova Conta          "  , "", "", "mv_cho","C",10,0,0,"G","", "", "", "", "MV_PAR24",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"25","Situacao (1)        "  , "", "", "mv_chp","C",01,0,0,"G","", "", "", "", "MV_PAR25",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "") 
//NOVO 01-12-2013- THIAGO
//NOVO 02-12-2013- THIAGO 
PutSx1(cPerg,"26","Natureza de         "  , "", "", "mv_chq","C",10,0,0,"G","", "", "", "", "MV_PAR26",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
PutSx1(cPerg,"27","Natureza Ate        "  , "", "", "mv_chr","C",10,0,0,"G","", "", "", "", "MV_PAR27",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
//NOVO 02-12-2013- THIAGO 
PutSx1(cPerg,"28","Conceder desconto de"  , "", "", "mv_chs","N",11,2,0,"G","", "", "", "", "MV_PAR28",""        , "", "", ""        , "", "", ""     , "", "", "", "", "", "", "", "", "", {"", "", "", ""}, {"", "", "", ""}, {"", "", ""}, "")
Return