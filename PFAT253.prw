...
ฑฑบPrograma  ณPfat253   บAutor  ณDanilo Pala         บ Data ณ  20130813   บฑฑ
...
@ 045,005 BUTTON "Executar" SIZE 40,12 ACTION Processa({||ProcArq()})
@ 045,060 BUTTON "Fechar" SIZE 40,12 ACTION ( Close(oDlg252) )
...
Static Function ProcArq()
	...
	If DBSeek(xFilial("SB1")+  alltrim(cProduto))
		If substr(cOpcao,1,1) =="I" //inserir
			If MsgYesNo("Este produto jแ existe. Deseja repor o estoque do produto "+ cProduto +"?")
				ProcRepor(cProduto, Val(Alltrim(cQtd)))
				cMsg := "Estoque do produto "+ Alltrim(cProduto) + " reposto"
			Else
				cMsg := "Produto jแ existente, estoque nใo reposto, opera็ใo cancelada pelo usuแrio"
			Endif
		Else
	...
	Else 
		...
		Else
			If MsgYesNo("Este produto nใo existe. Deseja inserir o produto "+ cProduto +"?")
				ProcInsert(cProduto, cEdicao)
				//MsgAlert("Repor o estoque do produto "+ cProduto)
				ProcRepor(cProduto, Val(Alltrim(cQtd)))
				cMsg := "Produto "+ Alltrim(cProduto) + " inserido e estoque reposto"
			Else
				cMsg := "Produto nใo existente, estoque nใo reposto, opera็ใo cancelada pelo usuแrio"	
			Endif
		Endif
	EndIf
	MsgInfo(cMsg)
