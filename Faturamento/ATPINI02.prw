...
ฑฑบPrograma  ณATPINI02  บ Autor ณ Marcio Torresson   บ Data ณ  14/08/08   บฑฑ
...
elseif mv_par01==2
	...
	MsgAlert("Esta op็ใo foi desabilitada, o programa considerarแ Exposi็ใo") //desabilitar 20110815
...
elseif cConsEmp ="C"
	...
	cCadastro := "Gera็ใo de NFs de Devolu็ใo e Venda: EXPOSICAO"
endif
...
elseif cConsEmp =="C"
	...
	cT := "Manuten็ใo de Vendas/Devolu็ใo: EXPOSICAO"
endif
Private aC        := {}
Private aR        := {}
Private aCGD      := {}
...
if xtipo == 4 .or. xtipo == 2
   ...
   if dbseek(xfilial("SA1")+(cAlias)->(zzy_client+zzy_loja))
      cCgc := a1_cgc
      if empty(cCgc)
         msginfo("Nใo ้ possํvel efetuar a emissใo de Nota Fiscal Simbolica para Vendedor sem CPF/CNPJ !!!")
         return
      endif
	  ...
	  if dbseek(xfilial("SA3")+SA1->A1_VEND)
			...
			if dbseek(xfilial("SA2")+SA3->A3_FORNECE+SA3->A3_LOJA)
				...
				msginfo("No cadastro do Vendedor informe o codigo do Fornecedor para emissใo da Nota Fiscal Simbolica/Transporte!!!")
				return
			endif
      else
         msginfo("No cadastro do Cliente informe o codigo do Vendedor, e no Vendedor informe o Fornecedor, para emissใo da Nota Fiscal Simbolica!!!")
         return
      endif
   endif
endif
...
while !eof() .and. (cAlias)->(ZZY_filial+ZZY_doc+ZZY_serie+ZZY_client+ZZY_loja) == cProcura
	...
	elseif (!empty((cAlias)->ZZY_nfsimb) .and. xtipo==4)
		 cLibera := "Jแ emitida Nota Simbolica deste produto, verifique!!!"
	else
...
if !eof() .and. c5_cliente == m->zzy_nclien .and. c5_lojacli == m->zzy_nloja
	IF SC5->C5_LIBEROK=="S" 	//20120529
		MSGALERT("ATENวรO: ESTE PEDIDO Jม FOI FATURADO!")
	ELSE
		nVlVend := c5_vlrped
	ENDIF
endif
        ...
        if aCols[nX][7] > 0     // ZZY_qtdven
	        ...
	        elseif cConsEmp =="C"
		        ...
		        aadd(aLinha,{"D1_TES","030",Nil})
	           	posicione("SF4",1,xfilial("SF4")+"018","F4_CODIGO")
	        endif
		    aadd(aLinha,{"D1_NFORI",aCols[nX][3],NIL})
	        aadd(aLinha,{"D1_SERIORI",aCols[nX][4],NIL})
            aadd(aLinha,{"D1_ITEMORI",aCols[nX][1],NIL})
...
if !dbseek(xfilial("SA1")+m->ZZY_nclien+m->ZZY_nloja)
	MsgAlert("Cliente nใo localizado!")
endif
...
elseif xtipo == 3 .and. m->ZZY_qtdarm > 0
	...
	*/                 
	...
	if nQTran < 0
		MsgInfo("Quantidade de Armazenagem Informada ้ Superior a quantidade Transportada, verifique!!!",cTitulo)
		aCols[n][8]:= 0
	endif		
...
elseif xtipo == 4 .and. m->ZZY_qtdsim > 0
	...
	if nq3fim < 0
		MsgInfo("Quantidade de Simbolica Informada ้ Superior ao Saldo Existente, verifique!!!",cTitulo)
		nq3fim := noldq3fim
		aCols[n][7]:= 0 //m->ZZY_qtdarm := 0
	endif		
...
elseif xtipo == 3 .and. m->ZZY_qtdarm < 0                   
	...
	lret := .f.	
elseif xtipo == 4 .and. m->ZZY_qtdsim < 0                  
	MsgInfo("Nใo ้ permitido valores negativos para NF Simbolica, verifique!!!",cTitulo)
	lret := .f.	
endif
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณTransAlmoxบAutor  ณDanilo PAla         บ Data ณ  20100922   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDesc.     ณ											                  บฑฑ
ฑฑบ          ณ                                                            บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
...
Private nQuant := 0
    For nX := 1 To len(aCols)
        if aCols[nX][8] > 0     // ZZY_qtdarm
	        ...
	        If nQuant >0
	        	//ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
		        //ณ Pega a variavel que identifica se o calculo do custo e' :    ณ
		        //ณ               O = On-Line                                    ณ
	    	    //ณ               M = Mensal                                     ณ
	    	    //ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	    	    ...
	    	    PRIVATE aRegSD3  := {}
	    	    //ฺฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฟ
	    	    //ณ Verifica se o custo medio e' calculado On-Line               ณ
	    	    //ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	    	    If cCusMed == "O"
...
/*
-----------------------------------------------------------------------------
ฑฑณFun??o    ณA260Processa  ณ Eveli Morasco             ณ Data ณ 16/01/92 ณฑฑ
-----------------------------------------------------------------------------
ฑฑณDescri??o ณ Processamento da inclusao                                  ณฑฑ
-----------------------------------------------------------------------------
ฑฑณParametrosณExpC01: Codigo do Produto Origem - Obrigatorio              ณฑฑ
ฑฑณ          ณExpC02: Almox Origem             - Obrigatorio              ณฑฑ
ฑฑณ          ณExpN01: Quantidade 1a UM         - Obrigatorio              ณฑฑ
ฑฑณ          ณExpC03: Documento                - Obrigatorio              ณฑฑ
ฑฑณ          ณExpD01: Data                     - Obrigatorio              ณฑฑ
ฑฑณ          ณExpN02: Quantidade 2a UM                                    ณฑฑ
ฑฑณ          ณExpC04: Sub-Lote                 - Obrigatorio se Rastro "S"ณฑฑ
ฑฑณ          ณExpC05: Lote                     - Obrigatorio se usa Rastroณฑฑ
ฑฑณ          ณExpD02: Validade                 - Obrigatorio se usa Rastroณฑฑ
ฑฑณ          ณExpC06: Numero de Serie                                     ณฑฑ
ฑฑณ          ณExpC07: Localizacao Origem                                  ณฑฑ
ฑฑณ          ณExpC08: Codigo do Produto Destino- Obrigatorio              ณฑฑ
ฑฑณ          ณExpC09: Almox Destino            - Obrigatorio              ณฑฑ
ฑฑณ          ณExpC10: Localizacao Destino                                 ณฑฑ
ฑฑณ          ณExpL01: Indica se movimento e estorno                       ณฑฑ
ฑฑณ          ณExpN03: Numero do registro original (utilizado estorno)     ณฑฑ
ฑฑณ          ณExpN04: Numero do registro destino (utilizado estorno)      ณฑฑ
ฑฑณ          ณExpC11: Indicacao do programa que originou os lancamentos   ณฑฑ
ฑฑณ          ณExpC12: cEstFis    - Estrutura Fisica          (APDL)       ณฑฑ
ฑฑณ          ณExpC13: cServico   - Servico                   (APDL)       ณฑฑ
ฑฑณ          ณExpC14: cTarefa    - Tarefa                    (APDL)       ณฑฑ
ฑฑณ          ณExpC15: cAtividade - Atividade                 (APDL)       ณฑฑ
ฑฑณ          ณExpC16: cAnomalia  - Houve Anomalia? (S/N)     (APDL)       ณฑฑ
ฑฑณ          ณExpC17: cEstDest   - Estrututa Fisica Destino  (APDL)       ณฑฑ
ฑฑณ          ณExpC18: cEndDest   - Endereco Destino          (APDL)       ณฑฑ
ฑฑณ          ณExpC19: cHrInicio  - Hora Inicio               (APDL)       ณฑฑ
ฑฑณ          ณExpC20: cAtuEst    - Atualiza Estoque? (S/N)   (APDL)       ณฑฑ
ฑฑณ          ณExpC21: cCarga     - Numero da Carga           (APDL)       ณฑฑ
ฑฑณ          ณExpC22: cUnitiza   - Numero do Unitizador      (APDL)       ณฑฑ
ฑฑณ          ณExpC23: cOrdTar    - Ordem da Tarefa           (APDL)       ณฑฑ
ฑฑณ          ณExpC24: cOrdAti    - Ordem da Atividade        (APDL)       ณฑฑ
ฑฑณ          ณExpC25: cRHumano   - Recurso Humano            (APDL)       ณฑฑ
ฑฑณ          ณExpC26: cRFisico   - Recurso Fisico            (APDL)       ณฑฑ
ฑฑณ          ณExpN05: nPotencia  - Potencia do Lote                       ณฑฑ
ฑฑณ          ณExpC27: cLoteDest  - Lote Destino da Transferencia          ณฑฑ
-----------------------------------------------------------------------------
ฑฑณ Uso      ณ Transferencia                                              ณฑฑ
-----------------------------------------------------------------------------
*/
Return     
...
ฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑฑ
ฑฑษออออออออออัออออออออออหอออออออัออออออออออออออออออออหออออออัอออออออออออออปฑฑ
ฑฑบPrograma  ณPINIP3PERGบ Autor ณ DANILO C S PALA    บ Data ณ  20100924   บฑฑ
ฑฑฬออออออออออุออออออออออสอออออออฯออออออออออออออออออออสออออออฯอออออออออออออนฑฑ
ฑฑบDescricao ณ 								 							  บฑฑ
ฑฑบ          ณ 															  บฑฑ
ฑฑฬออออออออออุออออออออออออออออออออออออออออออออออออออออออออออออออออออออออออนฑฑ
