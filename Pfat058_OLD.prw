...
╠╠ЁPrograma: PFAT058   ЁAutor: Raquel Farias          Ё Data:   27/09/99 Ё ╠╠
╠╠цддддддддддддддддддддаддддддддддддддддддддддддддддддадддддддддддддддддд╢ ╠╠
╠╠ЁDescri┤фo: Gera┤фo do arquivo de Assinantes  - revistas               Ё ╠╠
╠╠цдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢ ╠╠
╠╠ЁUso      : M╒dulo de Faturamento  Liberado para Usu═rio em:           Ё ╠╠
╠╠юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды ╠╠
╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠╠
╠╠зддддддддддддддддддддбддддддддддддддддддддддддддддддбдддддддддддддддддд© ╠╠
╠╠ЁAltera┤фo           ЁAlterado por: Raquel Farias   Ё Data:   10/04/00 Ё ╠╠
╠╠цддддддддддддддддддддаддддддддддддддддддддддддддддддадддддддддддддддддд╢ ╠╠
╠╠ЁDescri┤фo: Substitui c6_edfin por c6_edvenc a pedido da Solange       Ё ╠╠
╠╠цдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд╢ ╠╠
╠╠ЁLiberado para Usu═rio em: 10/04/00                                    Ё ╠╠
╠╠юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды ╠╠
...
SetPrvt("_ACAMPOS,_CNOME,CINDEX,MDTINCL")
...
SetPrvt("MITEMANT,MDESCR,MPEDRENOV,MPAGA1")
// PARAMETROS EDITORA PINI
//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё Variaveis utilizadas para parametros                                     Ё
//Ё mv_par01             // Cod.da Revista        (C-4)                      Ё
//Ё mv_par02             // Edicao Inicial        (N-4)                      Ё
//Ё mv_par03             // Edicao Final          (N-4)                      Ё
//Ё mv_par04             // Cod.da Revista        (C-4)                      Ё
//Ё mv_par05             // Edicao                (N-4)                      Ё
//Ё mv_par06             // Cod.da Revista        (C-4)                      Ё
//Ё mv_par07             // Edicao                (N-4)                      Ё
//Ё mv_par08             // Cod.da Revista        (C-4)                      Ё
//Ё mv_par09             // Edicao                (N-4)                      Ё
//Ё mv_par10             // Cod.da Revista        (C-4)                      Ё
//Ё mv_par11             // Edicao                (N-4)                      Ё
//Ё mv_par12             // Cod.da Revista        (C-4)                      Ё
//Ё mv_par13             // Edicao                (N-4)                      Ё
//Ё mv_par14             // Cod.da Revista        (C-4)                      Ё
//Ё mv_par15             // Edicao                (N-4)                      Ё
//Ё mv_par16             // Renova┤фo  Recupera┤фo Autom═tica Nenhum         Ё
//Ё mv_par17             // Cancelados Ativos-Repres Ativos-Sele┤фo  Nenhum  Ё
//Ё mv_par18             // Sim  Nфo                                         Ё
//Ё mv_par19             // M┬s/Ano                                          Ё
//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
// PARAMETROS PINI SISTEMAS
//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё Variaveis utilizadas para parametros                                     Ё
//Ё mv_par01             // Cod.do Produto Inicial (C-15)                    Ё
//Ё mv_par02             // Cod.do Produto Final   (C-15)                    Ё
//Ё mv_par03             // Edicao Inicial         (N-04)                    Ё
//Ё mv_par04             // Edicao Final           (N-04)                    Ё
//Ё mv_par05             // Renova┤фo  Recupera┤фo Automatica  Nenhum        Ё
//Ё mv_par06             // Cancelados Ativos-Repres Ativos-Sele┤фo  Nenhum  Ё
//Ё mv_par07             // Sim  Nфo                                         Ё
//Ё mv_par08             // M┬s/Ano                                          Ё
//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
cPerg     := "PFAT20"
MREV1     := ""
MED1      := ""
...
//зддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё Iinicio da grava┤фo do arquivo                                          Ё
//юддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
...
If MVXPAR16 == 1
	...
	DbGoTop()
	_cString := _cArqPath+"RENOV"+mEmpresa+".DBF"
	_cSeg    := _cArqPath+"RN"+mEmpresa+mMesano+".DBF"
Endif
...
If MVXPAR16 == 2
	...
	DbGoTop()
	_cString := _cArqPath+"RENOV"+mEmpresa+".DBF"
	_cSeg    := _cArqPath+"RC"+mEmpresa+mMesano+".DBF"
Endif
...
If MVXPAR16 == 3
	...
	DbGoTop()
	_cString := _cArqPath+"RENOV"+mEmpresa+".DBF"
	_cSeg    := _cArqPath+"RA"+mEmpresa+mMesano+".DBF"
Endif

If MVXPAR17 == 1
	DbSelectArea("PFAT058")
	DbGotop()
	_cString := _cArqPath+"ASSCANC"+Right(mEmpresa,1)+".DBF"
ElseIf MVXPAR17 == 2
	DbSelectArea("PFAT058")
	DbGotop()
	_cString := _cArqPath+"ASSAT"+mEmpresa+".DBF"
ElseIf MVXPAR17 == 3
	DbSelectArea("PFAT058")
	DbGoTop()
	_cString := _cArqPath+"ASSSEL"+mEmpresa+".DBF"
Endif
...
//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё  MELHORIAS:                                                  Ё
//Ё  CRIAR PARAMETRO-> MQTREV:=GETMV("MV_QTREV")                 Ё
//Ё  CRIAR MATRIZ 1 A MQTREV                                     Ё
//Ё  CRIAR NOVAS PERGUNTAS QUANDO NECESSARIO                     Ё
//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
	...
	IF MREVISTA <> '   '
		...
		*/
		...
		While !Eof() .and. SC6->C6_PRODUTO >= MPRODUTO1 .AND. SC6->C6_PRODUTO <= MPRODUTO2
			...
			//IF SC6->C6_NUM='030234'
			//	MSGALERT("ACHOU PEDIDO 030234")
			//ENDIF
			Do Case
			...
			IF MVXPAR17 == 2 .OR. MVXPAR17 == 3
				//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
				//Ё  VERIFICA INADIMPLENCIA.                                     Ё
				//Ё  ATUALIZA O CAMPO SITUAC NO ARQUIVO TEMPORARIO  E NO SC6     Ё
				//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
				IF MGRAT <> 'S'
...
//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
//Ё Deleta renovados e preenche campo c6_pedren,c6_itemren no sc6Ё
//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
MPEDIDO  := '  '
MITEM    := '  '
MPRODUTO := '  '
...
WHILE !EOF()
	...
	//здддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддд©
	//Ё Procura pedido anterior no sc6   ->c6_pedant e c6_itemant    Ё
	//юдддддддддддддддддддддддддддддддддддддддддддддддддддддддддддддды
	DBSELECTAREA("SC6")
	DbSetOrder(12)
