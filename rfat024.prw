...
Private mHora       := TIME()
Private wnrel       := "DIVC_" + SUBS(CUSUARIO,7,3)+SUBS(MHORA,1,2)+SUBS(MHORA,7,2)
Private nLastKey    := 0
...
//ภฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤฤู
	...
	MEND      := ALLTRIM(SA1->A1_TPLOG) + " " + ALLTRIM(SA1->A1_LOGR) + " " + ALLTRIM(SA1->A1_NLOGR) + " " + ALLTRIM(SA1->A1_COMPL) //20060322
	...
	IF MCODDEST # ' '
		...
		DBSELECTAREA("SZO")
		IF DBSEEK(XFILIAL()+MCLIENTE+MCODDEST)
			MEND    := ALLTRIM(SZO->ZO_TPLOG) + " " + ALLTRIM(SZO->ZO_LOGR) + " " + ALLTRIM(SZO->ZO_NLOGR) + " " + ALLTRIM(SZO->ZO_COMPL) //20060322
...
DBSELECTAREA("IVC")
_StringArq := "\SIGA\ETIQUETAS\ESPECIAIS\"+ SUBS(CUSUARIO,7,3)+SUBS(MHORA,1,2)+SUBS(MHORA,7,2) +".DBF"
COPY TO &_StringArq VIA "DBFCDXADS" // 20121106 
DBCLOSEAREA()     

DBSELECTAREA("AVERIG")
_StrArqAVERIG := "\SIGA\ETIQUETAS\ESPECIAIS\AVERIG" +SUBS(MHORA,7,2) +".DBF"
...
IF SA1->A1_ATIVIDA >= MV_PAR06 .AND. SA1->A1_ATIVIDA <= MV_PAR07 
 IF DTOS(SC6->C6_DATA) < '20030301' .AND. Mgrat <> "S"
	RETURN
 ELSE 
	DBSELECTAREA("ETIQP")
	Reclock("ETIQP",.t.)
