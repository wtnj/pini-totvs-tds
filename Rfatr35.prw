...
User Function Rfatr35()        // incluido pelo assistente de conversao do AP5 IDE em 25/02/02
...
SetPrvt("_MCONT,_MOTIVO,_MPERC,_MDESP,_CDESCR,AREGS")
SetPrvt("I,J,")
...
If AllTrim(MV_PAR05) #"."
   ...
   aAdd(_aCampos, { "VALPARC" ,"N", 12, 6 })
   aAdd(_aCampos, { "DESP_REM","N", 12, 2 })
...
_cInd    := CriaTrab(NIL,.F.)
_cKey    := "C6_FILIAL+C6_PRODUTO+C6_NUM+C6_ITEM+DTOS(C6_DATA)"
       ...
       If Found()
          ...
          Do While _mNum == SE1->E1_PEDIDO .And. !Eof()
             ...
             If SE1->E1_PARCELA == "A" .OR. SE1->E1_PARCELA == " "
                ...
                If Alltrim(MV_PAR05) #"." .And. _mDesp # 0
                   TRB->DESP_REM:= _mDesp
                EndIf
